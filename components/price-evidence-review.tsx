...
export function PriceEvidenceReview({ market, settlesAt }: { market: string; settlesAt: string }) {
  ...
  return <section className="draft-form">
    ...
    {evidence ? <div className="draft-ready"><strong>Candidate outcome: {evidence.record.decision.outcome.toUpperCase()}</strong><p>{evidence.record.decision.reason}</p><p>{evidence.note}</p><p>Collected: {evidence.record.publishedAt} · SHA-256: {evidence.evidenceHash}</p><p>Provider request: {evidence.providerUrl}</p><button type="button" onClick={download}>Download exact evidence JSON</button><details><summary>Original response and evidence record</summary><textarea readOnly rows={8} value={evidence.serialized} aria-label="Price evidence JSON" /></details><p>This does not propose or finalize a result. Publish the evidence durably and have the named resolver independently verify it before requesting any settlement signature.</p></div> : null}
  </section>;
}
